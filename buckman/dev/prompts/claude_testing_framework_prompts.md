...
- **Prompts 2–8** are implementation prompts, one per layer, issued after you review and approve each step
...
```
Your understanding of the pipeline looks [correct / mostly correct — here are corrections: ___].
...
Create a module (e.g., pipeline_manifest.py) that the main pipeline script calls at the end of every run. It should generate a manifest file (JSON or YAML, your pick — I want it human-readable) alongside the 5 report tables containing:
...
Also create a simple print_manifest_summary() function that prints a one-page human-readable summary to the console (e.g., "47 tests passed, 0 failed, 2 flags pending review, all input hashes verified").
```
...
This layer requires historical context. Based on what you found in the exploration step, here's what I need:
...
1. YEAR-OVER-YEAR RATE-OF-CHANGE:
   ...
   - [I'll set X and Y — for now use X=40% and Y=25% as defaults and make them configurable]
...
3. MULTI-YEAR TREND CHECK:
   - For key output values (total annual pumping, total annual depletion per reach), fit a simple linear regression to the prior available years
   - Compute 95% prediction interval
   - Flag if current year falls outside the PI

For all three checks: the thresholds should be configurable (in a config file or at the top of the module as named constants). Every flag should include the flagged value, the threshold, and a plain-English explanation of what it means.

If we don't have enough historical data in the project for some of these checks, tell me exactly what data I need to assemble and in what format, and stub out the check to accept that data once I provide it.
```
...
3. Ensure the Layer 5 regression harness (run_regression_2024.py) remains a SEPARATE standalone script. It should be run manually before deploying any pipeline changes, not as part of every production run.
